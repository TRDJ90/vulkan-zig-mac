...
pub const Swapchain = struct {
    ...
    pub fn init(gc: *const VulkanContext, allocator: Allocator, extent: vk.Extent2D) !Swapchain {
        return try initRecycle(gc, allocator, extent, .null_handle);
    }

    pub fn initRecycle(gc: *const VulkanContext, allocator: Allocator, extent: vk.Extent2D, old_handle: vk.SwapchainKHR) !Swapchain {
...
const SwapImage = struct {
    ...
    fn init(gc: *const VulkanContext, image: vk.Image, format: vk.Format) !SwapImage {
    ...
    fn deinit(self: SwapImage, gc: *const VulkanContext) void {
    ...
    fn waitForFence(self: SwapImage, gc: *const VulkanContext) !void {
        _ = try gc.dev.waitForFences(1, @ptrCast(&self.frame_fence), vk.TRUE, std.math.maxInt(u64));
    }
};

fn initSwapchainImages(gc: *const VulkanContext, swapchain: vk.SwapchainKHR, format: vk.Format, allocator: Allocator) ![]SwapImage {
...
fn findSurfaceFormat(gc: *const VulkanContext, allocator: Allocator) !vk.SurfaceFormatKHR {
...
fn findPresentMode(gc: *const VulkanContext, allocator: Allocator) !vk.PresentModeKHR {
