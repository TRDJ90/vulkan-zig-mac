...
pub fn main() !void {
    ...
    while (!window.shouldClose()) {
        ...
        zglfw.getFramebufferSize(window, &w, &h);
...
fn uploadVertices(gc: *const VulkanContext, pool: vk.CommandPool, buffer: vk.Buffer) !void {
...
fn copyBuffer(gc: *const VulkanContext, pool: vk.CommandPool, dst: vk.Buffer, src: vk.Buffer, size: vk.DeviceSize) !void {
    ...
    const cmdbuf = VulkanContext.CommandBuffer.init(cmdbuf_handle, gc.dev.wrapper);
...
fn destroyCommandBuffers(gc: *const VulkanContext, pool: vk.CommandPool, allocator: Allocator, cmdbufs: []vk.CommandBuffer) void {
    gc.dev.freeCommandBuffers(pool, @truncate(cmdbufs.len), cmdbufs.ptr);
    allocator.free(cmdbufs);
}

fn createFramebuffers(gc: *const VulkanContext, allocator: Allocator, render_pass: vk.RenderPass, swapchain: Swapchain) ![]vk.Framebuffer {
...
fn destroyFramebuffers(gc: *const VulkanContext, allocator: Allocator, framebuffers: []const vk.Framebuffer) void {
    for (framebuffers) |fb| gc.dev.destroyFramebuffer(fb, null);
    allocator.free(framebuffers);
}

fn createRenderPass(gc: *const VulkanContext, swapchain: Swapchain) !vk.RenderPass {
